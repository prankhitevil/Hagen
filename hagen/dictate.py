...
def recognise(pcm: np.ndarray, lang: str = "ru") -> str:
    ...
    Отметки времени диктовке не нужны, но просить их не вредно: модель
    голосового ввода — та же, что у файлов и «Перечитать точнее», и идёт тем
    же путём.
    """
    ...
    if arr.size < int(MIN_SECONDS * SR):
        return ""
    # Модели может не быть в папке (в сборку кладут одну, решение 16.09):
    # молча качать её посреди диктовки нельзя — человек ждал бы вставки текста
    # и не понимал, почему её нет. Модель — та, что выбрана для голосового
    # ввода (решение 21.09).
    part = asr.part_for("voice")
    if not needs.ready(part):
