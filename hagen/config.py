...
def _merge_asr_choice(data: dict[str, Any], raw: dict[str, Any]) -> dict[str, Any]:
    ...
    Умолчание — рекомендованное: одна точная модель на onnx-asr, полные веса;
    её же ставит установщик. Переносится только вариант, явно выбранный на
    стенде (asr_preset): он ложится на новые параметры. Делается при чтении,
    пока параметров нет в файле; первое же сохранение настроек запишет их явно.
