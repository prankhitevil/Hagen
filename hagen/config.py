...
DEFAULTS: dict[str, Any] = {
    ...
    # «Перечитать точнее», есть только когда звонки идут быстрой. Веса точной
    # модели asr_weights: сжатые («int8») или полные («fp32»). Действует после
    # перезапуска программы; нет файлов — роль берёт то, что скачано, и окно
    # говорит почему. По умолчанию — рекомендованное (asr.RECOMMENDED): одна
    # точная модель со сжатыми весами. Прежний ключ asr_engine («torch» или
    # «onnx_asr») не читается: движок теперь один (решение 23.09), а у кого
    # стоял torch, тот при чтении получает полные веса (_merge_asr_choice).
    "asr_count": 1,
    "asr_single": "precise",
    "asr_calls": "fast",
    "asr_voice": "precise",
    ...
    "asr_keep_parts": [],
    "asr_pending_delete": [],
    # Через сколько минут простоя выгружать из памяти модель, которая не служит
    # эфиру. Точная с полными весами занимает около 0,9 ГБ и нужна не всегда:
    # файлам, «Перечитать точнее» и диктовке. Модели эфира это не касается —
    # она держится всегда, иначе первая фраза записи ждала бы загрузку. В
    # вариантах одной модели так же держится точная: она и есть модель эфира.
    # 0 = не выгружать.
    "precise_idle_min": 30,
    ...
    # --- записи с телефона (23.09) ---
    # Папки, за которыми программа следит: новый звуковой или видеофайл в
    # любой из них распознаётся и размечается сам. Чем папка наполняется —
    # LocalSend, «Связь с телефоном», облачный диск, загрузки Telegram — дело
    # человека. %ПЕРЕМЕННЫЕ% раскрываются.
    "inbox_folders": [],
    # Страница для телефона в локальной сети: по QR-коду из настроек телефон
    # открывает её в браузере и отправляет файл. Свой порт, отдельный от
    # службы: та слушает только 127.0.0.1 и в сеть не выходит.
    "lan_enabled": False,
    "lan_port": 8788,
    # Адрес компьютера в сети для ссылки; пусто — первый подходящий.
    "lan_address": "",
    # Ключ в ссылке: без него страница не открывается. Секрет, наружу не отдаётся.
    "lan_key": "",
    "features": {
        ...
        "phone": False,            # записи с телефона: папки входящих и страница в сети
    },
    "todoist_token": "",
...
_SECRET_KEYS = {"hf_token", "api_keys", "todoist_token", "lan_key"}
...
#: Варианты на torch ложатся на точную с полными весами: движок теперь один, а
#: полные веса дают текст слово в слово как torch.
_PRESET_TO_CHOICE: dict[str, dict[str, Any]] = {
    "current": {"asr_count": 2, "asr_calls": "fast", "asr_voice": "precise",
                "asr_reread": True, "asr_weights": "fp32", "live_draft": True},
    "current_nodraft": {"asr_count": 2, "asr_calls": "fast", "asr_voice": "precise",
                        "asr_reread": True, "asr_weights": "fp32", "live_draft": False},
    "one_torch": {"asr_count": 1, "asr_single": "precise", "asr_weights": "fp32"},
    "one_torch_2": {"asr_count": 1, "asr_single": "precise", "asr_weights": "fp32"},
    "one_ox_fp32": {"asr_count": 1, "asr_single": "precise", "asr_weights": "fp32"},
    "one_ox_int8": {"asr_count": 1, "asr_single": "precise", "asr_weights": "int8"},
}
...
def _merge_asr_choice(data: dict[str, Any], raw: dict[str, Any]) -> dict[str, Any]:
    ...
    Умолчание — рекомендованное: одна точная модель, полные веса; её же ставит
    установщик. Переносится только вариант, явно выбранный на стенде
    (asr_preset): он ложится на новые параметры. Делается при чтении, пока
    параметров нет в файле; первое же сохранение настроек запишет их явно.

    Файл без выбора — это заводское, а не «как было»: раньше такой файл
    считался прежней установкой, и выходили две модели с torch. Но ровно такой
    файл пишет и установщик — в нём только папка хранилища, — и свежая
    установка показывала не ту модель, что скачана (22.09).

    Движка torch больше нет (решение 23.09): у кого он был выбран явно, тот
    молча переводится на полные веса — текст тот же слово в слово.
    """
    if not raw:
        return data
    if str(raw.get("asr_engine") or "") == "torch":
        data["asr_weights"] = "fp32"
    if any(key in raw for key in _ASR_CHOICE_KEYS):
        return data
    preset = str(raw.get("asr_preset") or "")
    if preset in _PRESET_TO_CHOICE:
        data.update(_PRESET_TO_CHOICE[preset])
    return data
