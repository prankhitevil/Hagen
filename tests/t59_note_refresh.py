...
try:
    with TestClient(server.app, base_url="http://127.0.0.1:8787") as cli:
        ...
        check("главные разделы: «О записи», протокол, саммари, стенограмма",
              ...
              == ["# О записи", "# Протокол", "# Саммари", "# Стенограмма"],
              [ln for ln in t4.splitlines() if ln.startswith("# ")])
        ...
        t6c = note_text(r6)
        check("«Сохранить стенограмму» добавляет стенограмму под документ",
        ...
        check("«Сохранить стенограмму»: документы, под ними стенограмма, участники",
              "под ними стенограмма с именами говорящих" in save_tip and "список участников" in save_tip, save_tip)
