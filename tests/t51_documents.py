...
Решения 13.09:
  ...
  * у каждого документа свой файл и свой раздел заметки — документы больше не
    затирают друг друга;
  * «Настройки → Документы»: задачу и структуру документа можно править, общие
...
try:
    ...
    say("")
    say("=== 2. Свои инструкции («Настройки → Документы») ===")
    ...
    with TestClient(server.app, base_url="http://127.0.0.1:8787") as cli:
        ...
        heads = [h for h in ("# Протокол", "# Краткое содержание", "# Конспект", "# Свои запросы", "# Стенограмма")
                 if ("\n" + h + "\n") in body]
        ...
        check("главные разделы: «О записи», 4 документа, стенограмма (формат 15.09)",
              mains == ["# О записи", "# Протокол", "# Краткое содержание", "# Конспект", "# Свои запросы",
                        "# Стенограмма"], mains)
        ...
        check("без заметки: главные разделы — «О записи», протокол и саммари",
              [ln for ln in tb.splitlines() if ln.startswith("# ")] == ["# О записи", "# Протокол", "# Краткое содержание"],
        ...
        check("главные разделы: «О записи», протокол, саммари, стенограмма",
              mains3 == ["# О записи", "# Протокол", "# Краткое содержание", "# Стенограмма"], mains3)
    ...
    check("вкладка «Документы» в настройках", 'id="prompt-list"' in html and 'id="set-owner"' in html)
