...
try:
    ...
    # Отметки времени диктовке не нужны, но путь — тот же, что у файлов и
    # «Перечитать точнее»: одна модель, один способ её звать.
    check("просим время слов — идём тем же путём, что файлы", calls[0][1] is True, calls)
    calls.clear()
