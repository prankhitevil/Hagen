...
для звонков и какая для голосового ввода; веса точной модели (сжатые или
полные). Программа сама считает, что скачать, что не нужно, и умеет
«Сбросить всё». Движок один — onnx-asr; прежний выбор torch переносится на
полные веса (решение 23.09).

Что проверяем — распознаватели подставные, поиск речи и дорожки эфира настоящие:
  1. Умолчания — рекомендованное (одна точная, полные веса); перенос прежнего
     выбора стенда (asr_preset) и прежнего движка torch; файл без выбора — в
     том числе тот, что пишет установщик, — даёт заводское, а не две модели.
  2. Роли: звонки, голосовой ввод и файлы идут своими движками; выбор
  ...
  7. Части моделей НА ВРЕМЕННЫХ ПАПКАХ: чего не хватает, что не нужно,
     ...
     хватает — отказ), общие файлы трёх моделей одной папки, удаление не
     выходит за папки моделей, отложенное удаление при запуске, «Сбросить всё»;
     файлы прежних версий (веса torch, свой экспорт в ONNX) — часть «unused».
...
calls = []
ready_state = {"fast": True, "ox_int8": True, "ox_fp32": True}
state = {"fails": False}
...
def fake_ox(pcm, engine):
    """Подставной onnx-asr: быстрая — без слов, точная — со словами (и отказом по просьбе)."""
    if engine == "fast":
        calls.append(("fast", engine, None))
        return asr.Result("быстрая модель")
    calls.append(("ox", engine, None))
    if state["fails"]:
...
REAL = {"ox": asr._transcribe_ox, "engine_ready": asr.engine_ready, "install": needs.install,
        "dirs": (needs.OLD_CKPT_DIR, needs.OLD_ONNX_DIR, asr.EN_DIR, asr.OX_DIR)}
...
def pick(**choice):
    ...
    Сценарии ниже написаны на полных весах: они проверяют роли, выгрузку и
    удаление, а не умолчание (его проверяет раздел 1)."""
    config.save(dict(asr.RECOMMENDED, **dict({"asr_calls": "fast", "asr_voice": "precise",
                                              "asr_reread": True, "asr_weights": "fp32"},
                                             **choice)))
...
try:
    ...
    want = asr.chosen()
    check("по умолчанию — одна точная модель, сжатые веса",
          want == {"live": "ox_int8", "voice": "ox_int8", "files": "ox_int8",
    ...
    check("выбора движка в настройках больше нет", "asr_engine" not in config.DEFAULTS
          and "asr_engine" not in asr.RECOMMENDED)

    real_path = config.SETTINGS_PATH
    tmpdir = Path(tempfile.mkdtemp(prefix="t88_settings_"))
    try:
        config.SETTINGS_PATH = tmpdir / "settings.json"
        # Варианты стенда на torch ложатся на точную с полными весами.
        cases = {"current": ("fast", "ox_fp32", "ox_fp32", True, True),
                 "current_nodraft": ("fast", "ox_fp32", "ox_fp32", True, False),
                 "one_torch": ("ox_fp32", "ox_fp32", "ox_fp32", False, False),
                 "one_torch_2": ("ox_fp32", "ox_fp32", "ox_fp32", False, False),
                 "one_ox_fp32": ("ox_fp32", "ox_fp32", "ox_fp32", False, False),
                 "one_ox_int8": ("ox_int8", "ox_int8", "ox_int8", False, False),
                 None: ("ox_int8", "ox_int8", "ox_int8", False, False)}
        ...
        config._cache = None
        got = asr.chosen()
        check("свежая установка: одна точная модель, сжатые веса",
              (got["live"], got["voice"], got["files"], got["reread"], got["drafts"])
              == ("ox_int8", "ox_int8", "ox_int8", False, False)
              and config.get("asr_count") == 1 and config.get("asr_weights") == "int8", got)
        raw = {"live_model": "v3_e2e_ctc", "asr_preset": "current", "asr_count": 2,
               "asr_calls": "precise", "asr_voice": "fast", "asr_weights": "int8"}
        config.SETTINGS_PATH.write_text(json.dumps(raw), encoding="utf-8")
        config._cache = None
        check("новые параметры в файле главнее прежнего asr_preset",
              asr.chosen()["live"] == "ox_int8" and asr.chosen()["voice"] == "fast", asr.chosen())
        # Хватает одного нового ключа: иначе заданное явно затиралось бы
        # переносом «как было».
        raw = {"live_model": "v3_e2e_ctc", "asr_weights": "fp32"}
        config.SETTINGS_PATH.write_text(json.dumps(raw), encoding="utf-8")
        config._cache = None
        got = asr.chosen()
        check("один новый ключ — переноса нет, остальное по умолчанию",
              (got["live"], got["voice"], got["files"]) == ("ox_fp32",) * 3, got)
        # У кого был выбран torch — молча полные веса, даже если стояли сжатые.
        raw = {"asr_count": 1, "asr_single": "precise", "asr_engine": "torch", "asr_weights": "int8"}
        config.SETTINGS_PATH.write_text(json.dumps(raw), encoding="utf-8")
        config._cache = None
        got = asr.chosen()
        check("прежний движок torch переносится на полные веса",
              (got["live"], got["voice"], got["files"]) == ("ox_fp32",) * 3
    ...
    check("одна точная: звонки, ввод и файлы — полные веса, со словами",
          calls == [("ox", "ox_fp32", None)] * 3 and len(res.words) == 3, calls)
    pick(asr_weights="int8")
    calls.clear()
    asr.transcribe_live(speech)
    check("сжатые веса — точная со сжатием", calls == [("ox", "ox_int8", None)], calls)
    ...
    route = pick(asr_count=2, asr_calls="fast", asr_voice="precise")
    calls.clear()
    asr.transcribe_live(speech)
    asr.transcribe_precise(speech, role="voice")
    asr.transcribe_precise(speech, role="files")
    check("две модели: звонки быстрой, ввод и файлы точной",
          [c[0] for c in calls] == ["fast", "ox", "ox"], calls)
    check("…и «Перечитать точнее» есть", route["reread"] is True, route)
    route = pick(asr_count=2, asr_calls="precise", asr_voice="fast")
    calls.clear()
    asr.transcribe_live(speech)
    asr.transcribe_precise(speech, role="voice")
    asr.transcribe_precise(speech, role="files")
    ...
    route = pick()
    check("нет и быстрой — берутся сжатые веса", route["live"] == "ox_int8", route)
    ...
    asr._ox_used.clear()
    pick()
    for engine in ("ox_fp32", "fast"):
        asr._ox_cache[engine] = object()
        asr._ox_used[engine] = time.time() - 10 * 3600
    got = asr.release_idle(minutes=30)
    check("модель эфира не выгружена, остальное — как раньше",
          got == ["fast"] and "ox_fp32" in asr._ox_cache, got)
    pick(asr_count=2, asr_calls="fast")
    asr._ox_used["ox_fp32"] = time.time() - 10 * 3600
    got = asr.release_idle(minutes=30)
    check("звонки на быстрой — точная выгружается после простоя", got == ["ox_fp32"], got)
    asr._ox_cache.clear()
    asr._ox_used.clear()
    ...
    check("в карточке записи — чем шёл эфир, процессор, задержка фраз",
          stand.get("title") == "Точная (полные веса)" and "delay_median_s" in stand
    ...
    needs.OLD_CKPT_DIR = tmp_models / "gigaam"
    needs.OLD_ONNX_DIR = tmp_models / "onnx"
    asr.EN_DIR = tmp_models / "onnx-asr"
    asr.OX_DIR = asr.EN_DIR / "gigaam-v3"
    roots = [needs.OLD_CKPT_DIR, needs.OLD_ONNX_DIR, asr.EN_DIR, asr.OX_DIR]
    ...
    stranger = [needs.OLD_ONNX_DIR / "чужой.onnx", asr.OX_DIR / "README.md", tmp_models / "рядом.txt"]
    for p in stranger:
    ...
    check("одна точная: скачивать нечего", st["missing"] == [] and st["download_mb"] == 0,
          st["missing"])
    check("не нужно: быстрая, сжатые веса, файлы прежних версий; английская не в счёт",
          [u["key"] for u in st["unneeded"]] == ["fast", "precise_ox_int8", "unused"],
          [u["key"] for u in st["unneeded"]])
    check("файлы прежних версий — веса torch и свой экспорт в ONNX",
          {p.name for p in needs.part_files("unused")} >= {"v3_e2e_rnnt.ckpt", "v3_e2e_ctc.ckpt",
                                                           "v3_e2e_ctc.onnx", "v3_e2e_rnnt_encoder.onnx"},
          sorted(p.name for p in needs.part_files("unused")))
    ...
    check("«Удалить»: занятое программой — после перезапуска",
          res["later"] == ["fast"] and set(res["removed"]) == {"precise_ox_int8", "unused"}, res)
    check("файлы удалены, занятое пока лежит",
          needs.on_disk("fast") and not needs.on_disk("precise_ox_int8") and not needs.on_disk("unused"))
    check("общие файлы остались — полные веса и быстрая живы",
          needs.ready("precise_ox_fp32") and needs.ready("fast") and (asr.OX_DIR / "config.json").exists()
          and (asr.OX_DIR / "v3_e2e_rnnt_vocab.txt").exists())
    ...
    put("fast")                      # быстрая жива: общий config.json должен остаться
    put("precise_ox_int8")
    ...
    check("удалили последний вариант точной — ушёл её словарь, config.json нужен быстрой",
          not (asr.OX_DIR / "v3_e2e_rnnt_vocab.txt").exists() and (asr.OX_DIR / "config.json").exists())
    needs.remove("fast")
    check("удалили и быструю — ушёл и общий config.json", not (asr.OX_DIR / "config.json").exists())
    ...
    # Программа работала на быстрой и полных весах: их файлы уйдут после
    # перезапуска, сейчас удалено всё прочее.
    check("«Сбросить всё»: кроме занятого, остались только сжатые веса",
          [k for k in needs.MODEL_PARTS if needs.on_disk(k)]
          == ["fast", "precise_ox_int8", "precise_ox_fp32"],
          ([k for k in needs.MODEL_PARTS if needs.on_disk(k)], res))
    check("…занятое работавшей программой — после перезапуска",
          set(res["later"]) == {"fast", "precise_ox_fp32"}, res)
    ...
    check("…после перезапуска осталась только одна модель",
          [k for k in needs.MODEL_PARTS if needs.on_disk(k)] == ["precise_ox_int8"],
          [k for k in needs.MODEL_PARTS if needs.on_disk(k)])
    for key in needs.MODEL_PARTS:
        put(key)
    needs.remove("precise_ox_int8")
    ...
    real_install = needs.PARTS["precise_ox_int8"]["install"]
    needs.PARTS["precise_ox_int8"]["install"] = broken
    try:
        needs.reset_models()
        ...
        needs.PARTS["precise_ox_int8"]["install"] = real_install
    ...
    for choice, want_parts in (({}, ["precise_ox_fp32"]),
                               ({"asr_count": 2, "asr_weights": "int8"}, ["fast", "precise_ox_int8"])):
        installed.clear()
        pick(**choice)
        ok = asr.prepare_all()
    ...
    ids = ["set-asr-count", "set-asr-single", "set-asr-calls", "set-asr-voice", "set-asr-reread",
           "set-live-draft", "set-asr-weights", "btn-asr-download",
    ...
    check("выбора движка (torch) на странице нет", "set-asr-engine" not in html and "torch" not in pane
          and "asr_engine" not in js)
    ...
    with TestClient(server.app, base_url="http://127.0.0.1:8787") as cli:
        ...
        check("«Удалить» через службу", r.status_code == 200
              and not needs.on_disk("unused") and needs.ready("precise_ox_fp32"), r.text[:200])
        check("старого маршрута вариантов нет", cli.get("/api/asr/presets").status_code == 404)
finally:
    asr._transcribe_ox = REAL["ox"]
    asr.engine_ready = REAL["engine_ready"]
    needs.install = REAL["install"]
    needs.OLD_CKPT_DIR, needs.OLD_ONNX_DIR, asr.EN_DIR, asr.OX_DIR = REAL["dirs"]
    live.DRAFT_EVERY_S = 1.5
    asr._live_route = None
